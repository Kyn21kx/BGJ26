...
public class HealthSystem : GameSystem {
	...
	public static Event<delegate void (BeefHush.Entity entityRef)> OnDeathEvent= default;
	...
	public void OnUpdate(float delta) {
		...
		this.m_damagedEntities.Each<Health, Damageable>(scope (entityRef, health, damageable) => {
			...
			//WARNING:Added for testing purposes
			health.value -= 1;

			if (health.value <= 0f && !this.m_entitiesToDestroy.Contains(entityRef.Id)) {
				// Die
				Console.WriteLine("Sending death Event");
				OnDeathEvent(entityRef);
