		...
		this.entityQuery.Each<PlayerTag, RigidBody, Controller, MovementStat>(scope (entityRef,
			 tag, rigidBody, controller, movementStat) => {
				...
				if(Hush.InputManager.IsKeyDown((EKeyCode)controller.right)){
				 ...
				 //TODO: Not the proper way of handling it
				if(Hush.InputManager.IsKeyUp((EKeyCode)controller.invB)){
					InventorySystem.backwardIndexInventory(entityRef);
				}
				 
				if(Hush.InputManager.IsKeyUp((EKeyCode)controller.invF)){
					InventorySystem.fowardIndexInventory(entityRef);
				}

				if(Hush.InputManager.IsKeyUp((EKeyCode)controller.pickup)){
				//	InventorySystem.addToSlot();
				}
